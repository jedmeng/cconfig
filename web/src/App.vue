  ...
  <div v-else class="layout" :class="{ collapsed: sidebarCollapsed }">
    <aside class="sidebar">
      ...
      <button
        ...
        :title="sidebarCollapsed ? '配置源' : ''"
        @click="goSourcesList"
      >
        <i class="bi bi-hdd-stack icon"></i>
        <span v-if="!sidebarCollapsed">配置源</span>
      </button>
      <button
        :class="{ active: isTabActive('modifiers') }"
        :title="sidebarCollapsed ? '修改器' : ''"
        @click="goModifiersList"
      >
        <i class="bi bi-sliders icon"></i>
        <span v-if="!sidebarCollapsed">修改器</span>
      </button>
      <button
        :class="{ active: isTabActive('schemes') }"
        :title="sidebarCollapsed ? '配置方案' : ''"
        @click="goSchemesList"
      >
        <i class="bi bi-diagram-3 icon"></i>
        <span v-if="!sidebarCollapsed">配置方案</span>
      </button>
      <div class="sidebar-footer">
        <SidebarAccount
          ...
          @logout="logout"
        />
        <div v-if="!sidebarCollapsed" class="sidebar-footnote">Powered by Jedm</div>
      </div>
      <button
        class="collapse-btn"
